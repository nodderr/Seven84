(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))i(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const r of o.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&i(r)}).observe(document,{childList:!0,subtree:!0});function s(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(a){if(a.ep)return;a.ep=!0;const o=s(a);fetch(a.href,o)}})();const v=[{path:"",label:"Home"},{path:"gallery",label:"Gallery"},{path:"events",label:"Events"},{path:"performances",label:"Performances"},{path:"results",label:"Results"},{path:"journey",label:"Our Journey"},{path:"about",label:"About Us"}];function w(){const e=document.getElementById("navbar"),t=window.location.hash.slice(1)||"";e.className="navbar",e.innerHTML=`
  ...
  `,requestAnimationFrame(()=>s.classList.add("active")),document.getElementById("lightbox-close").addEventListener("click",m),s.addEventListener("click",i=>{i.target===s&&m()}),document.getElementById("lightbox-prev").addEventListener("click",i=>{i.stopPropagation(),p(-1)}),document.getElementById("lightbox-next").addEventListener("click",i=>{i.stopPropagation(),p(1)}),document.addEventListener("keydown",f),document.body.style.overflow="hidden"}function p(e){l=(l+e+n.length)%n.length;const t=document.querySelector(".lightbox-image"),s=document.querySelector(".lightbox-counter");t.style.opacity="0",setTimeout(()=>{t.src=n[l].src,t.alt=n[l].title||"",s.textContent=`${l+1} / ${n.length}`,t.style.opacity="1"},150)}function f(e){e.key==="Escape"&&m(),e.key==="ArrowLeft"&&p(-1),e.key==="ArrowRight"&&p(1)}function m(){const e=document.getElementById("lightbox-overlay");e.classList.remove("active"),setTimeout(()=>{e.classList.add("hidden"),e.innerHTML=""},300),document.removeEventListener("keydown",f),document.body.style.overflow=""}function I(e){const t=document.getElementById("video-modal");t.classList.remove("hidden"),t.innerHTML=`
  ...
  `,requestAnimationFrame(()=>t.classList.add("active")),document.getElementById("video-close").addEventListener("click",u),t.addEventListener("click",s=>{s.target===t&&u()}),document.addEventListener("keydown",b),document.body.style.overflow="hidden"}function b(e){e.key==="Escape"&&u()}function u(){const e=document.getElementById("video-modal");e.classList.remove("active"),setTimeout(()=>{e.classList.add("hidden"),e.innerHTML=""},300),document.removeEventListener("keydown",b),document.body.style.overflow=""}const c=[],S=[{id:"all",label:"All"},{id:"live",label:"Live Performances"},{id:"cover",label:"Covers"}];function A(){const e=c.find(t=>t.featured)||c[0]||{title:"New Performance Coming Soon",youtubeId:""};return`
  ...
  `}function F(){const e=c.find(a=>a.featured)||c[0],t=c.filter(a=>a.id!==e.id),s=S.map((a,o)=>`
  ...
  `}function G(e){const t=d.find(i=>i.id===e);if(!t)return`
  ...
  `}function N(e){const t=String(e).toLowerCase();return t.includes("1st")||t.includes("winner")||t==="first"?"badge-gold":t.includes("2nd")||t.includes("runner")?"badge-silver":t.includes("3rd")?"badge-bronze":"badge-neutral"}window.appAPI={openLightbox:e=>L(y,e),openVideo:e=>I(e)};const h={"":A,gallery:M,about:P,journey:j,performances:F,events:O,results:H};function g(){const e=document.getElementById("page-content"),t=window.location.hash.slice(1),[s,i]=t.split("?");if(s&&!h[s]){window.location.hash="";return}const a=new URLSearchParams(i||"");e.style.opacity="0",setTimeout(()=>{const o=h[s]||h[""];e.innerHTML=o(a),window.scrollTo({top:0,behavior:"instant"}),e.style.opacity="1",x()},150)}function K(){w(),E(),window.addEventListener("hashchange",g),g()}document.addEventListener("DOMContentLoaded",K);
