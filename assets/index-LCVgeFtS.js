(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&n(l)}).observe(document,{childList:!0,subtree:!0});function t(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=t(s);fetch(s.href,i)}})();const g=[{path:"",label:"Home"},{path:"events",label:"Events"},{path:"performances",label:"Performances"},{path:"results",label:"Results"},{path:"journey",label:"Our Journey"},{path:"about",label:"About Us"}];function S(){const a=document.getElementById("navbar"),e=window.location.hash.slice(1)||"";a.className="navbar",a.innerHTML=`
  ...
  `}const b=a=>a.split(" ")[0],y=a=>[...a].sort((e,t)=>{const n=d.findIndex(i=>i.name===e||i.name.startsWith(e)),s=d.findIndex(i=>i.name===t||i.name.startsWith(t));return n-s});function j(){const a=c.find(s=>s.featured)||c[0],e=c.filter(s=>s.id!==a.id),t=D.map((s,i)=>`
    ...
    <div class="performance-card reveal" data-category="${s.category}" onclick="window.appAPI.openVideo('${s.youtubeId}')">
      ...
      <div class="performance-info">
        ...
        <div class="performer-tags">
          ${y(s.bandMembers||[]).map(i=>`<span class="performer-tag">${b(i)}</span>`).join("")}
        </div>
        ${s.eventId?`
  ...
  `}function G(a){const e=u.find(n=>n.id===a);if(!e)return`
    ...
    <div class="page-container page-event-detail fade-in">
      ...
      ${e.youtubeId?`
        <section class="event-main-media slide-up" style="animation-delay: 0.2s">
          ...
          <div class="performer-tags">
            ${q(e.bandMembers||[]).map(n=>`<span class="performer-tag">${P(n)}</span>`).join("")}
          </div>
        </section>
      `:""}
  ...
  `}function F(){const a=u.filter(e=>e.type==="competition");return`
  ...
  `}function K(a){const e=String(a).toLowerCase();return e.includes("1st")||e.includes("winner")||e==="first"?"badge-gold":e.includes("2nd")||e.includes("runner")?"badge-silver":e.includes("3rd")?"badge-bronze":"badge-neutral"}window.appAPI={openEventSpecificLightbox:(a,e)=>_(a,e),openVideo:a=>A(a)};const p={"":M,about:E,journey:T,performances:j,events:R,results:F};function f(){const a=document.getElementById("page-content"),e=window.location.hash.slice(1),[t,n]=e.split("?");if(t&&!p[t]){window.location.hash="";return}const s=new URLSearchParams(n||"");a.style.opacity="0",setTimeout(()=>{const i=p[t]||p[""];a.innerHTML=i(s),window.scrollTo({top:0,behavior:"instant"}),a.style.opacity="1",x()},150)}function z(){S(),I(),window.addEventListener("hashchange",f),f()}document.addEventListener("DOMContentLoaded",z);
