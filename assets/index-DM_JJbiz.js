(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))n(t);new MutationObserver(t=>{for(const r of t)if(r.type==="childList")for(const l of r.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&n(l)}).observe(document,{childList:!0,subtree:!0});function s(t){const r={};return t.integrity&&(r.integrity=t.integrity),t.referrerPolicy&&(r.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?r.credentials="include":t.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function n(t){if(t.ep)return;t.ep=!0;const r=s(t);fetch(t.href,r)}})();const y=[{path:"",label:"Home"},{path:"events",label:"Events"},{path:"performances",label:"Performances"},{path:"results",label:"Results"},{path:"journey",label:"Our Journey"},{path:"about",label:"About Us"}];function V(){const a=document.getElementById("navbar");let e=window.location.pathname;e.startsWith("/")&&(e=e.slice(1)),e.endsWith("/")&&(e=e.slice(0,-1)),a.className="navbar",a.innerHTML=`
  ...
  `,document.querySelectorAll("[data-nav]").forEach(t=>{t.addEventListener("click",r=>{r.preventDefault();const l=t.getAttribute("data-nav");window.appAPI.navigate(l),u()})});const n=document.getElementById("nav-toggle");n.addEventListener("click",()=>{n.classList.toggle("open"),s.classList.toggle("open"),document.body.style.overflow=s.classList.contains("open")?"hidden":""}),window.addEventListener("resize",()=>{window.innerWidth>768&&s.classList.contains("open")&&u()}),k(a)}function u(){const a=document.getElementById("nav-toggle"),e=document.getElementById("mobile-overlay");a&&e&&(a.classList.remove("open"),e.classList.remove("open"),document.body.style.overflow="")}function k(a){let e=!1;window.addEventListener("scroll",()=>{e||(window.requestAnimationFrame(()=>{window.scrollY>50?a.classList.add("scrolled"):a.classList.remove("scrolled"),e=!1}),e=!0)})}function E(){let a=window.location.pathname;a.startsWith("/")&&(a=a.slice(1)),a.endsWith("/")&&(a=a.slice(0,-1)),document.querySelectorAll(".navbar-link").forEach(e=>{e.getAttribute("data-nav")===a?e.classList.add("active"):e.classList.remove("active")})}function N(){const a=document.getElementById("site-footer");a.className="footer",a.innerHTML=`
  ...
  `,a.querySelectorAll("[data-nav]").forEach(e=>{e.addEventListener("click",s=>{s.preventDefault();const n=e.getAttribute("data-nav");window.appAPI.navigate(n),window.scrollTo({top:0,behavior:"smooth"})})})}let i=[],o=0;function _(a,e=0){i=a,o=e;const s=document.getElementById("lightbox-overlay");s.classList.remove("hidden"),s.innerHTML=`
  ...
  `}const F=[{date:"August 2023",title:"Beginning the Journey",description:"The first sparks of Seven.84. A group of musicians from different backgrounds coming together in a college dorm room to create a new sound — a fusion of Hindustani classical depth and Bollywood energy.",image:null}],$=[{number:"15",suffix:"+",label:"Live Shows"},{number:"5",suffix:"",label:"Awards Won"},{number:"8",suffix:"",label:"Band Members"},{number:"50",suffix:"k+",label:"Digital Reach"}];function x(){const a=$.map(s=>`
  ...
  `).join(""),n=e.map(t=>`
    <div class="performance-card reveal" data-category="${t.category}" onclick="window.appAPI.openVideo('${t.youtubeId}')">
      ...
      <div class="performance-info">
        ...
        ${t.eventId?`
          <button class="btn btn-ghost btn-sm mt-sm" onclick="event.stopPropagation(); window.appAPI.navigate('/events?id=${t.eventId}')">
            View Event Details
          </button>
        `:""}
      </div>
  ...
  `}const q=a=>[...a].sort((e,s)=>{const n=d.findIndex(r=>r.name===e||r.name.startsWith(e)),t=d.findIndex(r=>r.name===s||r.name.startsWith(s));return n-t});function O(){const a=v.filter(e=>e.type==="competition");return`
  ...
  `}function H(a){const e=String(a).toLowerCase();return e.includes("1st")||e.includes("winner")||e==="first"?"badge-gold":e.includes("2nd")||e.includes("runner")?"badge-silver":e.includes("3rd")?"badge-bronze":"badge-neutral"}window.appAPI={openEventSpecificLightbox:(a,e)=>_(a,e),openVideo:a=>A(a),scrollHighlights:a=>{const e=document.getElementById("infinite-reel-scroll");if(e){const s=e.clientWidth*.8;e.scrollBy({left:a*s,behavior:"smooth"})}},navigate:a=>{const e=a.startsWith("/")?a:"/"+a;window.location.pathname+window.location.search!==e&&(history.pushState(null,"",e),window.dispatchEvent(new Event("popstate")))},initInfiniteScroll:()=>{const a=document.getElementById("infinite-reel-scroll");if(!a)return;const e=a.querySelectorAll(".highlight-card");if(e.length===0)return;const s=e[0].offsetWidth+parseInt(getComputedStyle(a).gap),n=e.length-8;a.scrollLeft=s*4,a.addEventListener("scroll",()=>{const t=a.scrollLeft;t>=s*(n+6)?a.scrollTo({left:s*6,behavior:"instant"}):t<=s*2&&a.scrollTo({left:s*(n+2),behavior:"instant"})})}};const h={"":R,about:W,journey:x,performances:j,events:L,results:O};function C(){const a=document.getElementById("page-content");let e=window.location.pathname;e.startsWith("/")&&(e=e.slice(1)),e.endsWith("/")&&(e=e.slice(0,-1)),e&&!h[e]&&(history.replaceState(null,"","/"),e="");const s=window.location.search,n=new URLSearchParams(s||"");a.style.opacity="0",setTimeout(()=>{const t=h[e]||h[""];a.innerHTML=t(n),(!e||e==="")&&window.appAPI.initInfiniteScroll();const r={"":"Seven.84 — Hindustani Bollywood Fusion",about:"Seven.84 — About Us",journey:"Seven.84 — Our Journey",performances:"Seven.84 — Performances",events:"Seven.84 — Events & Diary",results:"Seven.84 — Competitions & Results"};document.title=r[e]||r[""],window.scrollTo({top:0,behavior:"instant"}),a.style.opacity="1",E()},150)}function z(){V(),N(),window.addEventListener("popstate",C),document.addEventListener("click",a=>{const e=a.target.closest("a");if(e&&!e.target&&e.host===window.location.host){const s=e.getAttribute("href");if(s&&(s.startsWith("/")||s.startsWith("#"))){a.preventDefault();const n=s.startsWith("#")?s.slice(1):s;window.appAPI.navigate(n)}}}),C()}document.addEventListener("DOMContentLoaded",z);
