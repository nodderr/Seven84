(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const r of s)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function t(s){const r={};return s.integrity&&(r.integrity=s.integrity),s.referrerPolicy&&(r.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?r.credentials="include":s.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function n(s){if(s.ep)return;s.ep=!0;const r=t(s);fetch(s.href,r)}})();const u=[{path:"",label:"Home"},{path:"events",label:"Events"},{path:"performances",label:"Performances"},{path:"results",label:"Results"},{path:"journey",label:"Our Journey"},{path:"about",label:"About Us"}];function k(){const a=document.getElementById("navbar");let e=window.location.pathname;e.startsWith("/")&&(e=e.slice(1)),e.endsWith("/")&&(e=e.slice(0,-1)),a.className="navbar",a.innerHTML=`
  ...
  `,document.querySelectorAll("[data-nav]").forEach(s=>{s.addEventListener("click",r=>{r.preventDefault();const y="/"+s.getAttribute("data-nav");window.location.pathname!==y&&(history.pushState(null,"",y),window.dispatchEvent(new Event("popstate"))),m()})});const n=document.getElementById("nav-toggle");n.addEventListener("click",()=>{n.classList.toggle("open"),t.classList.toggle("open"),document.body.style.overflow=t.classList.contains("open")?"hidden":""}),window.addEventListener("resize",()=>{window.innerWidth>768&&t.classList.contains("open")&&m()}),E(a)}function m(){const a=document.getElementById("nav-toggle"),e=document.getElementById("mobile-overlay");a&&e&&(a.classList.remove("open"),e.classList.remove("open"),document.body.style.overflow="")}function E(a){let e=!1;window.addEventListener("scroll",()=>{e||(window.requestAnimationFrame(()=>{window.scrollY>50?a.classList.add("scrolled"):a.classList.remove("scrolled"),e=!1}),e=!0)})}function N(){let a=window.location.pathname;a.startsWith("/")&&(a=a.slice(1)),a.endsWith("/")&&(a=a.slice(0,-1)),document.querySelectorAll(".navbar-link").forEach(e=>{e.getAttribute("data-nav")===a?e.classList.add("active"):e.classList.remove("active")})}function _(){const a=document.getElementById("site-footer");a.className="footer",a.innerHTML=`
  ...
  `,a.querySelectorAll("[data-nav]").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();const n=e.getAttribute("data-nav");window.location.hash=n,window.scrollTo({top:0,behavior:"smooth"})})})}let l=[],i=0;function A(a,e=0){l=a,i=e;const t=document.getElementById("lightbox-overlay");t.classList.remove("hidden"),t.innerHTML=`
    <span class="lightbox-close" id="lightbox-close">✕</span>
    <span class="lightbox-nav lightbox-prev" id="lightbox-prev">‹</span>
    <img class="lightbox-image" src="${l[i].src}" alt="${l[i].title||""}" />
    <span class="lightbox-nav lightbox-next" id="lightbox-next">›</span>
    <div class="lightbox-counter">${i+1} / ${l.length}</div>
  `,requestAnimationFrame(()=>t.classList.add("active")),document.getElementById("lightbox-close").addEventListener("click",g),t.addEventListener("click",n=>{n.target===t&&g()}),document.getElementById("lightbox-prev").addEventListener("click",n=>{n.stopPropagation(),p(-1)}),document.getElementById("lightbox-next").addEventListener("click",n=>{n.stopPropagation(),p(1)}),document.addEventListener("keydown",M),document.body.style.overflow="hidden"}function p(a){i=(i+a+l.length)%l.length;const e=document.querySelector(".lightbox-image"),t=document.querySelector(".lightbox-counter");e.style.opacity="0",setTimeout(()=>{e.src=l[i].src,e.alt=l[i].title||"",t.textContent=`${i+1} / ${l.length}`,e.style.opacity="1"},150)}function M(a){a.key==="Escape"&&g(),a.key==="ArrowLeft"&&p(-1),a.key==="ArrowRight"&&p(1)}function g(){const a=document.getElementById("lightbox-overlay");a.classList.remove("active"),setTimeout(()=>{a.classList.add("hidden"),a.innerHTML=""},300),document.removeEventListener("keydown",M),document.body.style.overflow=""}function R(a){const e=document.getElementById("video-modal");e.classList.remove("hidden"),e.innerHTML=`
      ...
      <section class="section section-darker">
        ...
        <div class="highlight-reel">
          ...
          <div class="highlight-scroll" id="infinite-reel-scroll">
            ${(()=>{const e=c.slice(0,10),t=e.slice(-4),n=e.slice(0,4),s=[...t,...e,...n];return s.map((r,o)=>`
                <div class="highlight-card ${o<4||o>=s.length-4?"is-clone":""}" 
                     data-index="${o}"
                     onclick="window.appAPI.openVideo('${r.youtubeId}')">
                   <img src="${f(r)}" alt="${r.title}" loading="lazy" />
        ...
        <div class="container" style="text-align: center; max-width: 800px;">
          <div class="reveal-up">
            ...
            <div class="featured-video-wrapper" style="margin: 0 auto var(--space-xl);" onclick="window.appAPI.openVideo('${a.youtubeId}')">
               <img src="${f(a)}" alt="${a.title}" class="featured-video-thumb" />
  ...
  `}const w=a=>{if(a.eventId){const e=v.find(t=>t.id===a.eventId);if(e&&e.thumbnail)return e.thumbnail}return`https://img.youtube.com/vi/${a.youtubeId}/hqdefault.jpg`},I=a=>a.split(" ")[0],B=a=>[...a].sort((e,t)=>{const n=d.findIndex(r=>r.name===e||r.name.startsWith(e)),s=d.findIndex(r=>r.name===t||r.name.startsWith(t));return n-s});function L(){const a=c.find(s=>s.featured)||c[0],e=c.filter(s=>s.id!==a.id),t=D.map((s,r)=>`
    <button class="filter-tab ${r===0?"active":""}" data-filter="${s.id}">
      ${s.label}
    </button>
  `).join(""),n=e.map(s=>`
    <div class="performance-card reveal" data-category="${s.category}" onclick="window.appAPI.openVideo('${s.youtubeId}')">
      <div class="performance-thumb-wrapper">
        <img src="${w(s)}" alt="${s.title}" class="performance-thumb" loading="lazy" onerror="this.src='https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?w=600&q=80'" />
      ...
      <div class="performance-info">
        ...
        <div class="performer-tags">
          ${B(s.bandMembers||[]).map(r=>`<span class="performer-tag">${I(r)}</span>`).join("")}
        </div>
        ${s.eventId?`
        ...
        <div class="container">
          ...
          <div class="featured-performance reveal-scale">
            ...
            <div class="featured-video-wrapper" onclick="window.appAPI.openVideo('${a.youtubeId}')">
              ...
              <img src="${w(a)}" alt="${a.title}" class="featured-video-thumb" onerror="this.src='https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=1600&q=80'" />
              <div class="featured-play-btn"></div>
            </div>
  ...
  `}function P(){const a=document.querySelectorAll(".filter-tab"),e=document.querySelectorAll(".performance-card");a.forEach(t=>{t.addEventListener("click",()=>{a.forEach(s=>s.classList.remove("active")),t.classList.add("active");const n=t.getAttribute("data-filter");e.forEach(s=>{n==="all"||s.getAttribute("data-category")===n?(s.style.display="block",setTimeout(()=>{s.style.opacity="1",s.style.transform="translateY(0)"},50)):(s.style.opacity="0",s.style.transform="translateY(10px)",setTimeout(()=>{s.style.display="none"},300))})})})}const T=a=>a.split(" ")[0],C=a=>[...a].sort((e,t)=>{const n=d.findIndex(r=>r.name===e||r.name.startsWith(e)),s=d.findIndex(r=>r.name===t||r.name.startsWith(t));return n-s});function K(a){const e=a?a.get("id"):null;return e?q(e):G()}function G(){return`
  ...
  `}function q(a){const e=v.find(n=>n.id===a);if(!e)return`
    ...
    <div class="page-container page-event-detail fade-in">
      ...
      ${e.youtubeId?`
        <section class="event-main-media slide-up" style="animation-delay: 0.2s">
          ...
          <div class="performer-tags">
            ${C(e.bandMembers||[]).map(n=>`<span class="performer-tag">${T(n)}</span>`).join("")}
          </div>
        </section>
  ...
  `}const O=a=>[...a].sort((e,t)=>{const n=d.findIndex(r=>r.name===e||r.name.startsWith(e)),s=d.findIndex(r=>r.name===t||r.name.startsWith(t));return n-s});function H(){const a=v.filter(e=>e.type==="competition");return`
            ...
            <tbody>
              ${a.length>0?a.map((e,t)=>`
                <tr class="result-row" onclick="const details = document.getElementById('details-${t}'); details.style.display = details.style.display === 'table-row' ? 'none' : 'table-row'; this.classList.toggle('active-row');">
                  ...
                  <td>
                    ${e.rank?`<span class="result-badge ${z(e.rank)}">${e.rank}</span>`:'<span class="opacity-50">—</span>'}
                  </td>
                </tr>
                  ...
                  <td colspan="5" class="result-details-cell">
                    <div class="result-details-content">
                      <div class="result-members">
                        ...
                        <span class="opacity-80">${e.bandMembers&&e.bandMembers.length>0?O(e.bandMembers).join(", "):"Lineup not specified"}</span>
                      </div>
  ...
  `}function z(a){const e=String(a).toLowerCase();return e.includes("1st")||e.includes("winner")||e==="first"?"badge-gold":e.includes("2nd")||e.includes("runner")?"badge-silver":e.includes("3rd")?"badge-bronze":"badge-neutral"}window.appAPI={openEventSpecificLightbox:(a,e)=>A(a,e),openVideo:a=>R(a),scrollHighlights:a=>{const e=document.getElementById("infinite-reel-scroll");if(e){const t=e.clientWidth*.8;e.scrollBy({left:a*t,behavior:"smooth"})}},initInfiniteScroll:()=>{const a=document.getElementById("infinite-reel-scroll");if(!a)return;const e=a.querySelectorAll(".highlight-card");if(e.length===0)return;const t=e[0].offsetWidth+parseInt(getComputedStyle(a).gap),n=e.length-8;a.scrollLeft=t*4,a.addEventListener("scroll",()=>{const s=a.scrollLeft;s>=t*(n+6)?a.scrollTo({left:t*6,behavior:"instant"}):s<=t*2&&a.scrollTo({left:t*(n+2),behavior:"instant"})})}};const h={"":W,about:F,journey:j,performances:L,events:K,results:H};function S(){const a=document.getElementById("page-content");let e=window.location.pathname;e.startsWith("/")&&(e=e.slice(1)),e.endsWith("/")&&(e=e.slice(0,-1)),e&&!h[e]&&(history.replaceState(null,"","/"),e="");const t=window.location.search,n=new URLSearchParams(t||"");a.style.opacity="0",setTimeout(()=>{const s=h[e]||h[""];a.innerHTML=s(n),(!e||e==="")&&window.appAPI.initInfiniteScroll();const r={"":"Seven.84 — Hindustani Bollywood Fusion",about:"Seven.84 — About Us",journey:"Seven.84 — Our Journey",performances:"Seven.84 — Performances",events:"Seven.84 — Events & Diary",results:"Seven.84 — Competitions & Results"};document.title=r[e]||r[""],window.scrollTo({top:0,behavior:"instant"}),a.style.opacity="1",N()},150)}function U(){k(),_(),window.addEventListener("popstate",S),S()}document.addEventListener("DOMContentLoaded",U);
