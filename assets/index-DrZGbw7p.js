(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const l of r.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&s(l)}).observe(document,{childList:!0,subtree:!0});function t(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(n){if(n.ep)return;n.ep=!0;const r=t(n);fetch(n.href,r)}})();const m=[{path:"",label:"Home"},{path:"events",label:"Events"},{path:"performances",label:"Performances"},{path:"results",label:"Results"},{path:"journey",label:"Our Journey"},{path:"about",label:"About Us"}];function x(){const a=document.getElementById("navbar");let e=window.location.pathname;e.startsWith("/")&&(e=e.slice(1)),e.endsWith("/")&&(e=e.slice(0,-1)),a.className="navbar",a.innerHTML=`
    <div class="navbar-inner">
      ...
      <div class="navbar-links">
        ${m.map(n=>`
          <a class="navbar-link ${e===n.path?"active":""}" data-nav="${n.path}">
            ${n.label}
          </a>
        `).join("")}
      </div>
  ...
  `,document.querySelectorAll("[data-nav]").forEach(n=>{n.addEventListener("click",r=>{r.preventDefault();const l=n.getAttribute("data-nav");window.appAPI.navigate(l),f()})});const s=document.getElementById("nav-toggle");s.addEventListener("click",()=>{s.classList.toggle("open"),t.classList.toggle("open"),document.body.style.overflow=t.classList.contains("open")?"hidden":""}),window.addEventListener("resize",()=>{window.innerWidth>768&&t.classList.contains("open")&&f()}),j(a)}function f(){const a=document.getElementById("nav-toggle"),e=document.getElementById("mobile-overlay");a&&e&&(a.classList.remove("open"),e.classList.remove("open"),document.body.style.overflow="")}function j(a){let e=!1;window.addEventListener("scroll",()=>{e||(window.requestAnimationFrame(()=>{window.scrollY>50?a.classList.add("scrolled"):a.classList.remove("scrolled"),e=!1}),e=!0)})}function L(){let a=window.location.pathname;a.startsWith("/")&&(a=a.slice(1)),a.endsWith("/")&&(a=a.slice(0,-1)),document.querySelectorAll(".navbar-link").forEach(e=>{e.getAttribute("data-nav")===a?e.classList.add("active"):e.classList.remove("active")})}function P(){const a=document.getElementById("site-footer");a.className="footer",a.innerHTML=`
  ...
  `,a.querySelectorAll("[data-nav]").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();const s=e.getAttribute("data-nav");window.appAPI.navigate(s),window.scrollTo({top:0,behavior:"smooth"})})})}let i=[],o=0;function K(a,e=0){i=a,o=e;const t=document.getElementById("lightbox-overlay");t.classList.remove("hidden"),t.innerHTML=`
  ...
  `,requestAnimationFrame(()=>t.classList.add("active")),document.getElementById("lightbox-close").addEventListener("click",g),t.addEventListener("click",s=>{s.target===t&&g()}),document.getElementById("lightbox-prev").addEventListener("click",s=>{s.stopPropagation(),p(-1)}),document.getElementById("lightbox-next").addEventListener("click",s=>{s.stopPropagation(),p(1)}),document.addEventListener("keydown",E),document.body.style.overflow="hidden"}function p(a){o=(o+a+i.length)%i.length;const e=document.querySelector(".lightbox-image"),t=document.querySelector(".lightbox-counter");e.style.opacity="0",setTimeout(()=>{e.src=i[o].src,e.alt=i[o].title||"",t.textContent=`${o+1} / ${i.length}`,e.style.opacity="1"},150)}function E(a){a.key==="Escape"&&g(),a.key==="ArrowLeft"&&p(-1),a.key==="ArrowRight"&&p(1)}function g(){const a=document.getElementById("lightbox-overlay");a.classList.remove("active"),setTimeout(()=>{a.classList.add("hidden"),a.innerHTML=""},300),document.removeEventListener("keydown",E),document.body.style.overflow=""}function G(a){const e=document.getElementById("video-modal");e.classList.remove("hidden"),e.innerHTML=`
      ...
      <section class="section section-darker">
        ...
        <div class="highlight-reel">
          ...
          <div class="highlight-scroll" id="infinite-reel-scroll">
            ${(()=>{const e=c.slice(0,10),t=e.slice(-4),s=e.slice(0,4),n=[...t,...e,...s];return n.map((r,l)=>`
                <div class="highlight-card ${l<4||l>=n.length-4?"is-clone":""}" 
                     data-index="${l}"
                     onclick="window.appAPI.openVideo('${r.youtubeId}')">
                   <img src="${w(r)}" alt="${r.title}" loading="lazy" />
        ...
        <div class="container" style="text-align: center; max-width: 800px;">
          <div class="reveal-up">
            ...
            <div class="featured-video-wrapper" style="margin: 0 auto var(--space-xl);" onclick="window.appAPI.openVideo('${a.youtubeId}')">
               <img src="${w(a)}" alt="${a.title}" class="featured-video-thumb" />
      ...
      <section class="section section-dark">
        <div class="container">
          ...
          <div class="members-roster">
            ${d.map((e,t)=>{const s=e.photo||O(e.name);return`
      <div class="member-tile" style="animation: fadeInUp 0.5s ${t*.05}s var(--ease-out) both">
        <div class="member-tile-img-wrap">
          <img src="${s}" alt="${e.name}" class="member-tile-img" loading="lazy" />
  ...
  `}const U=[{date:"August 2023",title:"Beginning the Journey",description:"The first sparks of Seven.84. A group of musicians from different backgrounds coming together in a college dorm room to create a new sound — a fusion of Hindustani classical depth and Bollywood energy.",image:null}],J=[{number:"50",suffix:"+",label:"Live Shows"},{number:"10",suffix:"+",label:"Awards Won"},{number:"17",suffix:"",label:"Band Members"},{number:"40",suffix:"k+",label:"Digital Reach"}];function Q(){const a=J.map(t=>`
  ...
  `).join(""),e=U.map((t,s)=>`
      <div class="timeline-item">
        ...
        <div class="timeline-content ${s%2!==0?"reveal-left":"reveal-right"}">
  ...
  `}function Y(){const a=document.getElementById("stats-bar");if(!a)return;const e=a.querySelectorAll(".stat-number");let t=!1;const s=()=>{t||(t=!0,e.forEach(r=>{const l=parseInt(r.dataset.count,10),A=r.dataset.suffix||"",_=1500,D=performance.now(),y=R=>{const W=R-D,u=Math.min(W/_,1),$=1-Math.pow(1-u,3),F=Math.round($*l);r.textContent=F+A,u<1&&requestAnimationFrame(y)};requestAnimationFrame(y)}))},n=new IntersectionObserver(r=>{r[0].isIntersecting&&(s(),n.disconnect())},{threshold:.3});n.observe(a)}const I=a=>{if(a.eventId){const e=v.find(t=>t.id===a.eventId);if(e&&e.thumbnail)return e.thumbnail}return`https://img.youtube.com/vi/${a.youtubeId}/hqdefault.jpg`},B=a=>a.split(" ")[0],T=a=>[...a].sort((e,t)=>{const s=d.findIndex(r=>r.name===e||r.name.startsWith(e)),n=d.findIndex(r=>r.name===t||r.name.startsWith(t));return s-n});function Z(){const a=c.find(n=>n.featured)||c[0],e=c.filter(n=>n.id!==a.id),t=q.map((n,r)=>`
    <button class="filter-tab ${r===0?"active":""}" data-filter="${n.id}">
      ${n.label}
    </button>
  `).join(""),s=e.map(n=>`
    <div class="performance-card reveal" data-category="${n.category}" onclick="window.appAPI.openVideo('${n.youtubeId}')">
      <div class="performance-thumb-wrapper">
        <img src="${I(n)}" alt="${n.title}" class="performance-thumb" loading="lazy" onerror="this.src='https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?w=600&q=80'" />
      ...
      <div class="performance-info">
        ...
        <div class="performer-tags">
          ${T(n.bandMembers||[]).map(r=>`<span class="performer-tag">${B(r)}</span>`).join("")}
        </div>
        ${n.eventId?`
          <button class="btn btn-ghost btn-sm mt-sm" onclick="event.stopPropagation(); window.appAPI.navigate('/events?id=${n.eventId}')">
            View Event Details
          </button>
        `:""}
      </div>
        ...
        <div class="container">
          ...
          <div class="featured-performance reveal-scale">
            ...
            <div class="featured-video-wrapper" onclick="window.appAPI.openVideo('${a.youtubeId}')">
              ...
              <img src="${I(a)}" alt="${a.title}" class="featured-video-thumb" onerror="this.src='https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=1600&q=80'" />
              <div class="featured-play-btn"></div>
            </div>
            ...
            <div class="featured-video-info">
              ...
              <div class="performer-tags">
                ${T(a.bandMembers||[]).map(n=>`<span class="performer-tag">${B(n)}</span>`).join("")}
              </div>
              ${a.eventId?`
                <button class="btn btn-outline btn-sm mt-md" onclick="event.stopPropagation(); window.appAPI.navigate('/events?id=${a.eventId}')">
  ...
  `}function X(){const a=document.querySelectorAll(".filter-tab"),e=document.querySelectorAll(".performance-card");a.forEach(t=>{t.addEventListener("click",()=>{a.forEach(n=>n.classList.remove("active")),t.classList.add("active");const s=t.getAttribute("data-filter");e.forEach(n=>{s==="all"||n.getAttribute("data-category")===s?(n.style.display="block",setTimeout(()=>{n.style.opacity="1",n.style.transform="translateY(0)"},50)):(n.style.opacity="0",n.style.transform="translateY(10px)",setTimeout(()=>{n.style.display="none"},300))})})})}const C=a=>a.split(" ")[0],S=a=>[...a].sort((e,t)=>{const s=d.findIndex(r=>r.name===e||r.name.startsWith(e)),n=d.findIndex(r=>r.name===t||r.name.startsWith(t));return s-n});function ee(a){const e=a?a.get("id"):null;return e?se(e):ae()}function ae(){return`
    <div class="page-container page-events fade-in">
      ...
      <section class="events-catalog slide-up" style="animation-delay: 0.2s">
        ...
        <div class="events-grid">
          ${v.filter(e=>e.youtubeId||e.youtubeIds&&e.youtubeIds.length>0||e.thumbnail||e.photos&&e.photos.length>0).map(e=>`
            <div class="event-card" onclick="window.appAPI.navigate('/events?id=${e.id}')">
  ...
  `}function se(a){const e=v.find(s=>s.id===a);if(!e)return`
    ...
    `;const t=e.photos&&e.photos.length>0;return window.appAPI.openEventLightbox=s=>{window.appAPI.openEventSpecificLightbox(e.photos,s)},`
    <div class="page-container page-event-detail fade-in">
      ...
      ${e.youtubeId?`
        <section class="event-main-media slide-up" style="animation-delay: 0.2s">
          ...
          <div class="performer-tags">
            ${S(e.bandMembers||[]).map(s=>`<span class="performer-tag">${C(s)}</span>`).join("")}
          </div>
        </section>
        ...
        <section class="event-main-media slide-up" style="animation-delay: 0.2s">
          ${e.youtubeIds.map(s=>`
            <div class="video-embed-container mb-xl">
              <iframe 
                src="https://www.youtube.com/embed/${s}?rel=0&vq=hd1080&modestbranding=1" 
                title="YouTube video player" 
                frameborder="0" 
                allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" 
                allowfullscreen>
      ...
      ${t?`
        <section class="event-gallery slide-up" style="animation-delay: 0.3s">
          ...
          <div class="gallery-masonry" style="--columns: 3;">
            ${e.photos.map((s,n)=>`
              <div class="gallery-item" onclick="appAPI.openEventLightbox(${n})">
                <img src="${s.src}" alt="Event Photo" loading="lazy" />
              </div>
            `).join("")}
          </div>
  ...
  `}const M=a=>[...a].sort((e,t)=>{const s=d.findIndex(r=>r.name===e||r.name.startsWith(e)),n=d.findIndex(r=>r.name===t||r.name.startsWith(t));return s-n});function te(){const a=v.filter(s=>s.type==="competition"),e=`
    <section class="results-table-container slide-up results-desktop" style="animation-delay: 0.2s">
      <div class="results-table-wrapper">
        <table class="results-table">
          ...
          <tbody>
            ${a.length>0?a.map((s,n)=>`
              <tr class="result-row" onclick="const details = document.getElementById('details-${n}'); details.style.display = details.style.display === 'table-row' ? 'none' : 'table-row'; this.classList.toggle('active-row');">
                <td class="font-accent text-saffron text-nowrap">
                  <span class="dropdown-icon">▶</span> ${s.date}
                </td>
                <td class="font-accent text-saffron text-nowrap">${s.time||""}</td>
                <td class="font-display size-md text-cream">${s.name}</td>
                <td class="opacity-80">${s.venue}</td>
                <td>
                  ${s.rank?`<span class="result-badge ${k(s.rank)}">${s.rank}</span>`:'<span class="opacity-50">—</span>'}
                </td>
              </tr>
              <tr id="details-${n}" class="result-details-row" style="display: none;">
                <td colspan="5" class="result-details-cell">
                  <div class="result-details-content">
                    <div class="result-members">
                      <strong class="text-saffron">Lineup:</strong>
                      <span class="opacity-80">${s.bandMembers&&s.bandMembers.length>0?M(s.bandMembers).join(", "):"Lineup not specified"}</span>
                    </div>
                    ${s.youtubeId||s.thumbnail||s.photos&&s.photos.length>0?`
                      <a href="/events?id=${s.id}" class="result-event-link">
    ...
    <section class="results-cards results-mobile" style="animation-delay: 0.2s">
      ${a.length>0?a.map((s,n)=>`
        <div class="result-card" onclick="const d = this.querySelector('.result-card-details'); d.classList.toggle('open'); this.classList.toggle('active');">
          <div class="result-card-header">
            <div class="result-card-info">
              <h3 class="result-card-title">${s.name}</h3>
              <div class="result-card-meta">
                <span class="text-saffron">${s.date}</span>
                ${s.time?`<span class="result-card-sep">·</span><span>${s.time}</span>`:""}
                <span class="result-card-sep">·</span><span>${s.venue}</span>
              </div>
            </div>
            ${s.rank?`<span class="result-badge ${k(s.rank)}">${s.rank}</span>`:""}
          </div>
  ...
  `}function k(a){const e=String(a).toLowerCase();return e.includes("1st")||e.includes("winner")||e==="first"?"badge-gold":e.includes("2nd")||e.includes("runner")?"badge-silver":e.includes("3rd")?"badge-bronze":"badge-neutral"}window.appAPI={openEventSpecificLightbox:(a,e)=>K(a,e),openVideo:a=>G(a),scrollHighlights:a=>{const e=document.getElementById("infinite-reel-scroll");if(e){const t=e.clientWidth*.8;e.scrollBy({left:a*t,behavior:"smooth"})}},navigate:a=>{const e=a.startsWith("/")?a:"/"+a;window.location.pathname+window.location.search!==e&&(history.pushState(null,"",e),window.dispatchEvent(new Event("popstate")))},initInfiniteScroll:()=>{const a=document.getElementById("infinite-reel-scroll");if(!a)return;const e=a.querySelectorAll(".highlight-card");if(e.length===0)return;const t=e[0].offsetWidth+parseInt(getComputedStyle(a).gap),s=e.length-8;a.scrollLeft=t*4,a.addEventListener("scroll",()=>{const n=a.scrollLeft;n>=t*(s+6)?a.scrollTo({left:t*6,behavior:"instant"}):n<=t*2&&a.scrollTo({left:t*(s+2),behavior:"instant"})})}};const h={"":H,about:z,journey:Q,performances:Z,events:ee,results:te};function V(){const a=document.getElementById("page-content");let e=window.location.pathname;e.startsWith("/")&&(e=e.slice(1)),e.endsWith("/")&&(e=e.slice(0,-1)),e&&!h[e]&&(history.replaceState(null,"","/"),e="");const t=window.location.search,s=new URLSearchParams(t||"");a.style.opacity="0",setTimeout(()=>{const n=h[e]||h[""];a.innerHTML=n(s),(!e||e==="")&&window.appAPI.initInfiniteScroll();const r={"":"Seven.84 — Hindustani Bollywood Fusion",about:"Seven.84 — About Us",journey:"Seven.84 — Our Journey",performances:"Seven.84 — Performances",events:"Seven.84 — Events & Diary",results:"Seven.84 — Competitions & Results"};document.title=r[e]||r[""],window.scrollTo({top:0,behavior:"instant"}),a.style.opacity="1",L()},150)}function ne(){x(),P(),window.addEventListener("popstate",V),document.addEventListener("click",a=>{const e=a.target.closest("a");if(e&&!e.target&&e.host===window.location.host){const t=e.getAttribute("href");if(t&&(t.startsWith("/")||t.startsWith("#"))){a.preventDefault();const s=t.startsWith("#")?t.slice(1):t;window.appAPI.navigate(s)}}}),V()}document.addEventListener("DOMContentLoaded",ne);
