(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))r(t);new MutationObserver(t=>{for(const n of t)if(n.type==="childList")for(const l of n.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&r(l)}).observe(document,{childList:!0,subtree:!0});function s(t){const n={};return t.integrity&&(n.integrity=t.integrity),t.referrerPolicy&&(n.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?n.credentials="include":t.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function r(t){if(t.ep)return;t.ep=!0;const n=s(t);fetch(t.href,n)}})();const b=[{path:"",label:"Home"},{path:"events",label:"Events"},{path:"performances",label:"Performances"},{path:"results",label:"Results"},{path:"journey",label:"Our Journey"},{path:"about",label:"About Us"}];function w(){const a=document.getElementById("navbar"),e=window.location.hash.slice(1)||"";a.className="navbar",a.innerHTML=`
  ...
  `,a.querySelectorAll("[data-nav]").forEach(t=>{t.addEventListener("click",n=>{n.preventDefault();const l=t.getAttribute("data-nav");window.location.hash=l,T()})});const s=document.getElementById("nav-toggle"),r=document.getElementById("mobile-overlay");s.addEventListener("click",()=>{s.classList.toggle("open"),r.classList.toggle("open"),document.body.style.overflow=r.classList.contains("open")?"hidden":""}),M(a)}function T(){const a=document.getElementById("nav-toggle"),e=document.getElementById("mobile-overlay");a&&e&&(a.classList.remove("open"),e.classList.remove("open"),document.body.style.overflow="")}function M(a){let e=!1;window.addEventListener("scroll",()=>{e||(window.requestAnimationFrame(()=>{window.scrollY>50?a.classList.add("scrolled"):a.classList.remove("scrolled"),e=!1}),e=!0)})}function k(){const a=window.location.hash.slice(1)||"";document.querySelectorAll(".navbar-link").forEach(e=>{e.getAttribute("data-nav")===a?e.classList.add("active"):e.classList.remove("active")})}function E(){const a=document.getElementById("site-footer");a.className="footer",a.innerHTML=`
  ...
  `,a.querySelectorAll("[data-nav]").forEach(e=>{e.addEventListener("click",s=>{s.preventDefault();const r=e.getAttribute("data-nav");window.location.hash=r,window.scrollTo({top:0,behavior:"smooth"})})})}let i=[],o=0;function V(a,e=0){i=a,o=e;const s=document.getElementById("lightbox-overlay");s.classList.remove("hidden"),s.innerHTML=`
  ...
  `,requestAnimationFrame(()=>s.classList.add("active")),document.getElementById("lightbox-close").addEventListener("click",g),s.addEventListener("click",r=>{r.target===s&&g()}),document.getElementById("lightbox-prev").addEventListener("click",r=>{r.stopPropagation(),v(-1)}),document.getElementById("lightbox-next").addEventListener("click",r=>{r.stopPropagation(),v(1)}),document.addEventListener("keydown",I),document.body.style.overflow="hidden"}function v(a){o=(o+a+i.length)%i.length;const e=document.querySelector(".lightbox-image"),s=document.querySelector(".lightbox-counter");e.style.opacity="0",setTimeout(()=>{e.src=i[o].src,e.alt=i[o].title||"",s.textContent=`${o+1} / ${i.length}`,e.style.opacity="1"},150)}function I(a){a.key==="Escape"&&g(),a.key==="ArrowLeft"&&v(-1),a.key==="ArrowRight"&&v(1)}function g(){const a=document.getElementById("lightbox-overlay");a.classList.remove("active"),setTimeout(()=>{a.classList.add("hidden"),a.innerHTML=""},300),document.removeEventListener("keydown",I),document.body.style.overflow=""}function S(a){const e=document.getElementById("video-modal");e.classList.remove("hidden"),e.innerHTML=`
      ...
      <section class="section section-darker">
        ...
        <div class="highlight-reel">
          ...
          <div class="highlight-scroll" id="infinite-reel-scroll">
            ${(()=>{const e=d.slice(0,10),s=e.slice(-3),r=e.slice(0,3),t=[...s,...e,...r];return t.map((n,l)=>`
                <div class="highlight-card ${l<3||l>=t.length-3?"is-clone":""}" 
                     data-index="${l}"
                     onclick="window.appAPI.openVideo('${n.youtubeId}')">
                  <img src="https://img.youtube.com/vi/${n.youtubeId}/maxresdefault.jpg" alt="${n.title}" loading="lazy" />
        ...
        <div class="container" style="text-align: center; max-width: 800px;">
          <div class="reveal-up">
            ...
            <div class="featured-video-wrapper" style="margin: 0 auto var(--space-xl);" onclick="window.appAPI.openVideo('${a.youtubeId}')">
              <img src="https://img.youtube.com/vi/${a.youtubeId}/maxresdefault.jpg" alt="${a.title}" class="featured-video-thumb" />
  ...
  `}const W=[{date:"August 2023",title:"Beginning the Journey",description:"The first sparks of Seven.84. A group of musicians from different backgrounds coming together in a college dorm room to create a new sound — a fusion of Hindustani classical depth and Bollywood energy.",image:null}],x=[{number:"15",suffix:"+",label:"Live Shows"},{number:"5",suffix:"",label:"Awards Won"},{number:"8",suffix:"",label:"Band Members"},{number:"50",suffix:"k+",label:"Digital Reach"}];function $(){const a=x.map(s=>`
  ...
  `}const y=a=>a.split(" ")[0],u=a=>[...a].sort((e,s)=>{const r=c.findIndex(n=>n.name===e||n.name.startsWith(e)),t=c.findIndex(n=>n.name===s||n.name.startsWith(s));return r-t});function R(){const a=d.find(t=>t.featured)||d[0],e=d.filter(t=>t.id!==a.id),s=N.map((t,n)=>`
    ...
    <div class="page-container page-events fade-in">
      ...
      <section class="events-catalog slide-up" style="animation-delay: 0.2s">
        ...
        <div class="events-grid">
          ${m.filter(e=>e.youtubeId||e.thumbnail||e.photos&&e.photos.length>0).map(e=>`
  ...
  `}function F(a){const e=m.find(r=>r.id===a);if(!e)return`
    ...
    `;const s=e.photos&&e.photos.length>0;return window.appAPI.openEventLightbox=r=>{window.appAPI.openEventSpecificLightbox(e.photos,r)},`
    <div class="page-container page-event-detail fade-in">
      ...
      ${e.youtubeId?`
        <section class="event-main-media slide-up" style="animation-delay: 0.2s">
          ...
          <div class="performer-tags">
            ${j(e.bandMembers||[]).map(r=>`<span class="performer-tag">${D(r)}</span>`).join("")}
          </div>
        </section>
      `:""}
  ...
  `}const K=a=>[...a].sort((e,s)=>{const r=c.findIndex(n=>n.name===e||n.name.startsWith(e)),t=c.findIndex(n=>n.name===s||n.name.startsWith(s));return r-t});function q(){const a=m.filter(e=>e.type==="competition");return`
                  ...
                  <td colspan="5" class="result-details-cell">
                    <div class="result-details-content">
                      <div class="result-members">
                        ...
                        <span class="opacity-80">${e.bandMembers&&e.bandMembers.length>0?K(e.bandMembers).join(", "):"Lineup not specified"}</span>
                      </div>
  ...
  `}function H(a){const e=String(a).toLowerCase();return e.includes("1st")||e.includes("winner")||e==="first"?"badge-gold":e.includes("2nd")||e.includes("runner")?"badge-silver":e.includes("3rd")?"badge-bronze":"badge-neutral"}window.appAPI={openEventSpecificLightbox:(a,e)=>V(a,e),openVideo:a=>S(a),scrollHighlights:a=>{const e=document.getElementById("infinite-reel-scroll");if(e){const s=e.clientWidth*.8;e.scrollBy({left:a*s,behavior:"smooth"})}},initInfiniteScroll:()=>{const a=document.getElementById("infinite-reel-scroll");if(!a)return;const e=a.querySelectorAll(".highlight-card");if(e.length===0)return;const s=e[0].offsetWidth+parseInt(getComputedStyle(a).gap),r=e.length-6;a.scrollLeft=s*3,a.addEventListener("scroll",()=>{const t=a.scrollLeft,n=a.scrollWidth-a.clientWidth;t>=n-5?a.scrollTo({left:s*3,behavior:"instant"}):t<=5&&a.scrollTo({left:s*r,behavior:"instant"})})}};const p={"":A,about:C,journey:$,performances:R,events:L,results:q};function f(){const a=document.getElementById("page-content"),e=window.location.hash.slice(1),[s,r]=e.split("?");if(s&&!p[s]){window.location.hash="";return}const t=new URLSearchParams(r||"");a.style.opacity="0",setTimeout(()=>{const n=p[s]||p[""];a.innerHTML=n(t),(!s||s==="")&&window.appAPI.initInfiniteScroll(),window.scrollTo({top:0,behavior:"instant"}),a.style.opacity="1",k()},150)}function G(){w(),E(),window.addEventListener("hashchange",f),f()}document.addEventListener("DOMContentLoaded",G);
