(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))t(r);new MutationObserver(r=>{for(const n of r)if(n.type==="childList")for(const l of n.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&t(l)}).observe(document,{childList:!0,subtree:!0});function s(r){const n={};return r.integrity&&(n.integrity=r.integrity),r.referrerPolicy&&(n.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?n.credentials="include":r.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function t(r){if(r.ep)return;r.ep=!0;const n=s(r);fetch(r.href,n)}})();const T=[{path:"",label:"Home"},{path:"events",label:"Events"},{path:"performances",label:"Performances"},{path:"results",label:"Results"},{path:"journey",label:"Our Journey"},{path:"about",label:"About Us"}];function j(){const a=document.getElementById("navbar");let e=window.location.pathname;e.startsWith("/")&&(e=e.slice(1)),e.endsWith("/")&&(e=e.slice(0,-1)),a.className="navbar",a.innerHTML=`
  ...
  `,requestAnimationFrame(()=>s.classList.add("active")),document.getElementById("lightbox-close").addEventListener("click",w),s.addEventListener("click",t=>{t.target===s&&w()}),document.getElementById("lightbox-prev").addEventListener("click",t=>{t.stopPropagation(),u(-1)}),document.getElementById("lightbox-next").addEventListener("click",t=>{t.stopPropagation(),u(1)}),document.addEventListener("keydown",R),document.body.style.overflow="hidden"}function u(a){o=(o+a+i.length)%i.length;const e=document.querySelector(".lightbox-image"),s=document.querySelector(".lightbox-counter");e.style.opacity="0",setTimeout(()=>{e.src=i[o].src,e.alt=i[o].title||"",s.textContent=`${o+1} / ${i.length}`,e.style.opacity="1"},150)}function R(a){a.key==="Escape"&&w(),a.key==="ArrowLeft"&&u(-1),a.key==="ArrowRight"&&u(1)}function w(){const a=document.getElementById("lightbox-overlay");a.classList.remove("active"),setTimeout(()=>{a.classList.add("hidden"),a.innerHTML=""},300),document.removeEventListener("keydown",R),document.body.style.overflow=""}function W(a){const e=document.getElementById("video-modal");e.classList.remove("hidden"),e.innerHTML=`
  ...
  `}function Y(){const a=document.getElementById("stats-bar");if(!a)return;const e=a.querySelectorAll(".stat-number");let s=!1;const t=()=>{s||(s=!0,e.forEach(n=>{const l=parseInt(n.dataset.count,10),p=n.dataset.suffix||"",h=1500,g=performance.now(),c=b=>{const y=b-g,B=Math.min(y/h,1),x=1-Math.pow(1-B,3),F=Math.round(x*l);n.textContent=F+p,B<1&&requestAnimationFrame(c)};requestAnimationFrame(c)}))},r=new IntersectionObserver(n=>{n[0].isIntersecting&&(t(),r.disconnect())},{threshold:.3});r.observe(a)}const M=a=>{if(a.eventId){const e=v.find(s=>s.id===a.eventId);if(e&&e.thumbnail)return e.thumbnail}return`https://img.youtube.com/vi/${a.youtubeId}/hqdefault.jpg`},V=a=>a.split(" ")[0],k=a=>[...a].sort((e,s)=>{const t=d.findIndex(n=>n.name===e||n.name.startsWith(e)),r=d.findIndex(n=>n.name===s||n.name.startsWith(s));return t-r});function Z(){const a=[...m].reverse(),e=a.find(n=>n.featured)||a[0],s=a.filter(n=>n.id!==e.id),t=H.map((n,l)=>`
  ...
  `}function _(a){const e=String(a).toLowerCase();return e.includes("1st")||e.includes("winner")||e==="first"?"badge-gold":e.includes("2nd")||e.includes("runner")?"badge-silver":e.includes("3rd")?"badge-bronze":"badge-neutral"}window.appAPI={openEventSpecificLightbox:(a,e)=>G(a,e),openVideo:a=>W(a),scrollHighlights:a=>{const e=document.getElementById("infinite-reel-scroll");if(e){const s=e.clientWidth*.8;e.scrollBy({left:a*s,behavior:"smooth"})}},navigate:a=>{const e=a.startsWith("/")?a:"/"+a;window.location.pathname+window.location.search!==e&&(history.pushState(null,"",e),window.dispatchEvent(new Event("popstate")))},initInfiniteScroll:()=>{const a=document.getElementById("infinite-reel-scroll");if(!a)return;const e=a.querySelectorAll(".highlight-card");if(e.length===0)return;const s=e[0].offsetWidth+parseInt(getComputedStyle(a).gap),t=e.length-8;a.scrollLeft=s*4,a.addEventListener("scroll",()=>{const r=a.scrollLeft;r>=s*(t+6)?a.scrollTo({left:s*6,behavior:"instant"}):r<=s*2&&a.scrollTo({left:s*(t+2),behavior:"instant"})})}};const f={"":q,about:O,journey:Q,performances:Z,events:ee,results:se};function ne(){return`
